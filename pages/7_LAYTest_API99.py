...
import requests # Para buscar dados da API
import datetime # Para o seletor de data

# --- INÍCIO: Funções da API (Adaptadas para o Streamlit) ---
# O token deve ser gerenciado de forma segura, ex: st.secrets
...
def drop_reset_index_api_original(df):
    df = df.dropna()
...
def obter_dados_github_api(file_path):
    """Função para obter dados CSV do repositório GitHub via API."""
    ...
    try:
        ...
        if 'download_url' not in data or data['download_url'] is None:
            st.error(f"Não foi possível obter o URL de download para: {file_path}. Resposta da API: {data}")
            return None
        ...
        # Usar io.BytesIO para ler o conteúdo binário da resposta no pandas
        df = pd.read_csv(io.BytesIO(content_response.content))
        # NÃO vamos chamar drop_reset_index aqui, para que o app principal possa tratar os NaNs.
        # df = drop_reset_index_api_original(df) # Removido
        return df
    except requests.exceptions.HTTPError as http_err:
        if http_err.response.status_code == 404:
            st.warning(f"Arquivo não encontrado na API: {file_path}. Pode ser que não haja jogos para a data selecionada ou o arquivo histórico não exista no caminho esperado.")
        else:
            st.error(f"Erro HTTP ao buscar dados da API ({file_path}): {http_err}. Detalhes: {http_err.response.text}")
        return None
    except requests.exceptions.RequestException as e:
        st.error(f"Erro de requisição ao buscar dados da API ({file_path}): {e}")
        return None
    except Exception as e:
        st.error(f"Erro inesperado ao processar dados da API ({file_path}): {e}")
        return None

# --- FIM: Funções da API ---


# --- Função Auxiliar para Carregar Dados (Mantida para Upload Local, caso precise no futuro) ---
# Esta função não será usada para carregar os dados principais, mas pode ser útil para outras finalidades.
...
def pre_calculate_all_vars(df):
    ...
    try:
        ...
        st.error(f"Erro de divisão por zero durante o cálculo das VARs. Verifique se há odds inválidas ou zero. Detalhe: {zde}")
        return None
...
def define_var_strategies(vars_dict):
    ...
    # --- Cole aqui as 228 definições de estrategia_X ---
    #Base Casa
...
st.header("Carregamento da Base Histórica (via API)")
# Usaremos a base de dados Betfair Exchange da API
historical_file_path = "Bases_de_Dados/Betfair/Base_de_Dados_Betfair_Exchange_Back_Lay.csv"
with st.spinner(f"Buscando e carregando dados históricos da API ({historical_file_path})..."):
    df_historico_original = obter_dados_github_api(historical_file_path)

if df_historico_original is not None:
    st.success(f"Base de dados histórica ({historical_file_path}) carregada com sucesso da API! ({len(df_historico_original)} linhas)")
else:
    st.error(f"Falha ao carregar a base de dados histórica da API: {historical_file_path}. Verifique as mensagens de erro acima.")
...
if df_historico_original is not None:
    ...
    if missing_cols:
        st.error(f"Colunas essenciais ausentes na base histórica da API: {', '.join(missing_cols)}. Não é possível continuar.")
        df_historico = None 
    else:
        df_historico = df_historico_original[df_historico_original['League'].isin(APPROVED_LEAGUES)].copy()
        if df_historico.empty:
            st.warning("Nenhum jogo da base histórica da API pertence às ligas aprovadas. O backtest será vazio.")
        else:
    ...
    if df_historico is not None and not df_historico.empty:
        ...
        if vars_dict_historico is None:
            ...
            if not var_strategy_list:
                ...
                # --- Seção de Jogos do Dia (via API) ---
                st.divider() 
                st.header("🔍 Análise dos Jogos do Dia (via API)")
                ...
                if not approved_combined_strategies:
                    ...
                    # Seletor de data para jogos do dia
                    selected_date = st.date_input(
                        "Selecione a data para buscar os jogos do dia:",
                        datetime.date.today(),
                        min_value=datetime.date(2020, 1, 1), # Defina um limite razoável
                        max_value=datetime.date.today() + datetime.timedelta(days=7) # Permitir alguns dias no futuro
                    )
                    
                    if st.button("Buscar e Analisar Jogos do Dia da API"):
                        if selected_date:
                            date_str = selected_date.strftime("%Y-%m-%d")
                            daily_games_file_path = f"Jogos_do_Dia/Betfair/Jogos_do_Dia_Betfair_Back_Lay_{date_str}.csv"
                            
                            with st.spinner(f"Buscando jogos do dia ({date_str}) da API... ({daily_games_file_path})"):
                                df_daily_original = obter_dados_github_api(daily_games_file_path)

                            if df_daily_original is not None:
                                st.success(f"Arquivo de jogos do dia '{daily_games_file_path}' carregado da API ({len(df_daily_original)} linhas).")
                                ...
                                # As colunas 'Time', 'Home', 'Away' são para display, não críticas para cálculo, mas bom ter
                                # if 'Time' not in df_daily_original.columns: missing_daily_cols.append('Time')
                                # if 'Home' not in df_daily_original.columns: missing_daily_cols.append('Home')
                                # if 'Away' not in df_daily_original.columns: missing_daily_cols.append('Away')


                                if missing_daily_cols:
                                     st.error(f"Colunas necessárias ({', '.join(missing_daily_cols)}) não encontradas nos jogos do dia da API. Não é possível gerar recomendações.")
                                     df_daily = None
                                else:
                                    ...
                                    if df_daily.empty and not df_daily_original.empty:
                                        st.warning(f"Nenhum jogo do dia ({date_str}) pertence às ligas aprovadas.")
                                    elif not df_daily.empty:
                                        st.info(f"Encontrados {len(df_daily)} jogos do dia ({date_str}) nas ligas aprovadas para análise.")
                                    else:
                                        st.info(f"Não há jogos do dia ({date_str}) nas ligas aprovadas para analisar ou o arquivo está vazio.")

                                if df_daily is not None and not df_daily.empty:
                                    st.subheader(f"📋 Recomendações para os Jogos do Dia ({date_str})")
                                    with st.spinner("Calculando variáveis VAR e aplicando filtros aprovados..."):
                                        ...
                                        if vars_dict_daily is None:
                                            ...
                                            cols_to_display_base = ['Time', 'League', 'Home', 'Away'] # Adicionar 'Date', 'Time' (horário) se existirem
                                            # Adicionar colunas de data e hora se existirem no CSV da API
                                            if 'Date' in df_daily.columns: cols_to_display_base.insert(0, 'Date')
                                            if 'Time' in df_daily.columns and 'Time' not in cols_to_display_base : cols_to_display_base.insert(1, 'Time')


                                            cols_exist_daily = [col for col in cols_to_display_base if col in df_daily.columns]
                                            if not cols_exist_daily: # Fallback se nenhuma das colunas de display existir
                                                st.warning("Nenhuma das colunas 'Date', 'Time', 'League', 'Home', 'Away' encontrada nos dados dos jogos do dia. A exibição será limitada.")
                                                        ...
                                                        try:
                                                            ...
                                                            if not df_daily_filtered.empty:
                                                                for idx, row in df_daily_filtered.iterrows():
                                                                    rec = {}
                                                                    if cols_exist_daily: # Apenas se houver colunas para mostrar
                                                                        rec = row[cols_exist_daily].to_dict()
                                                                    
                                                                    rec['Recomendação_CS'] = cs_lay_name_approved
                                                                    rec['Filtro_VAR_Aplicado'] = var_name
                                                                    daily_recommendations_list.append(rec)
                                                        except Exception as e_apply_daily:
                                                            st.warning(f"Erro ao aplicar filtro {var_name} (de {combined_name}) aos jogos do dia: {e_apply_daily}. Pulando.")
                                                    else:
                                            ...
                                            if daily_recommendations_list:
                                                ...
                                                ordered_cols = []
                                                if 'Date' in df_final_recommendations.columns: ordered_cols.append('Date')
                                                if 'Time' in df_final_recommendations.columns: ordered_cols.append('Time')
                                                if 'League' in df_final_recommendations.columns: ordered_cols.append('League')
                                                if 'Home' in df_final_recommendations.columns: ordered_cols.append('Home')
                                                if 'Away' in df_final_recommendations.columns: ordered_cols.append('Away')
                                                ordered_cols.extend(['Recomendação_CS', 'Filtro_VAR_Aplicado'])
                                                
                                                # Adicionar colunas restantes que não estão na lista ordenada
                                                for col in df_final_recommendations.columns:
                                                    if col not in ordered_cols:
                                                        ordered_cols.append(col)
                                                
                                                df_final_recommendations = df_final_recommendations[ordered_cols]


                                                if cols_exist_daily and len(cols_exist_daily) > 1: # Garante que há colunas para agrupar
                                                    group_cols = cols_exist_daily
                                                    df_grouped_recs = df_final_recommendations.groupby(group_cols).agg(
                                                        Recomendações_CS=('Recomendação_CS', lambda x: ', '.join(sorted(list(set(x))))),
                                                        Filtros_VAR_Aplicados=('Filtro_VAR_Aplicado', lambda x: ', '.join(sorted(list(set(x)))))
                                                    ).reset_index()
                                                    st.dataframe(df_grouped_recs)
                                                else: 
                                                     st.dataframe(df_final_recommendations)
                                            else:
                                                st.info(f"Nenhum jogo do dia ({date_str}) (nas ligas aprovadas) correspondeu aos filtros das estratégias aprovadas.")
                                elif df_daily is None: # Erro de coluna ou leitura
                                    st.error(f"Não foi possível processar os jogos do dia ({date_str}) devido a erros anteriores.")
                                # else: df_daily vazio, mensagem já mostrada
                            # else: Erro ao carregar df_daily_original da API, erro já mostrado
                # Fim da seção de jogos do dia
    elif df_historico is None:
         pass 
    else: 
        st.info("Base histórica vazia após filtro de ligas. Nenhum backtest realizado.")

elif df_historico_original is None and not TOKEN:
    st.error("Token da API do GitHub não configurado. Por favor, configure-o em st.secrets.")
# else: Erro ao carregar df_historico_original da API, erro já mostrado pela função
